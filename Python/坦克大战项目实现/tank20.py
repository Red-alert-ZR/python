...
class MainGame():  # 主函数类
    ...
    def creatWalls(self):
        ...
        for i in range(1, 6):
            ...
            MainGame.Wall_list.append(wall)
        for i in range(1, 4):
            wall = Wall(MainGame.SCREEN_WIDTH - 425, MainGame.SCREEN_HEIGHT - 50 * i)
            MainGame.Wall_list.append(wall)
        for i in range(1, 4):
            wall = Wall(425, MainGame.SCREEN_HEIGHT - 50 * i)
            MainGame.Wall_list.append(wall)
        for i in range(1, 4):
            wall = Wall(425 + i * 50, MainGame.SCREEN_HEIGHT - 150)
            MainGame.Wall_list.append(wall)
...
class MyTank(Tank):  # 我方坦克类
    def __init__(self, left, top):
        Tank.__init__(self, self.left, self.top)
