...
class MainGame:  # 主函数类
    ...
    # 要创建的敌方轻型坦克的数量
    EnemyTank_A_count = 3
    # 设置EnemyTank_A_count的存在参数
    EnemyTank_A_live = True
    # 存储所有敌方重型坦克
    EnemyTank_B_list = []
    # 要创建的敌方重型的数量
    EnemyTank_B_count = 2
    ...
    def startGame(self):
        ...
        while True:
            ...
            MainGame.window.blit(
                self.getTextSurface_L("剩余敌方坦克%d辆" % len(MainGame.EnemyTank_A_list + MainGame.EnemyTank_B_list)), (5, 5))
            if MainGame.MyTank_live > 0:
                MainGame.window.blit(
                    self.getTextSurface_R("第1关" + '   ' + "剩余我方坦克数量： %d" % MainGame.MyTank_live), (MainGame.SCREEN_WIDTH - 280, 0))
            # 调用展示墙壁的方法
            self.blitWalls_A()
            # 调用展示墙壁的方法
            self.blitWalls_B()
            ...
            if len(MainGame.EnemyTank_A_list) == 0:
                if MainGame.EnemyTank_A_live == True:
                    # 创建敌方坦克
                    self.creatEnemyTank_B()
                    MainGame.EnemyTank_A_live = False
                else:
                    # 展示敌方坦克
            ...
            _display.update()
            # 失败方法
            if MainGame.home.live == False or MainGame.MyTank_live == 0:
                self.blitGameOver()
                _display.update()
                ...
                return self.endGame()
            # 胜利方法
            if len(MainGame.EnemyTank_A_list + MainGame.EnemyTank_B_list) == 0:
                music = Music('TankWar-master/music/victory.mp3')
                music.play()
    ...
    def blitMyTank(self):
        ...
        if MainGame.TANK_P1 and MainGame.TANK_P1.live:
            ...
            # 调用碰撞墙壁A的方法
            MainGame.TANK_P1.hitWalls_A()
            # 调用碰撞墙壁B的方法
            MainGame.TANK_P1.hitWalls_B()
            # 调用碰撞到敌方坦克
    ...
    def creatWalls_A(self):
        for i in range(2, 8):
            ...
            MainGame.Wall_A_list.append(wall)
        for i in range(2, 8):
        ...
        for i in range(1, 4):
            ...
            MainGame.Wall_A_list.append(wall)
        for i in range(5, 8):
            wall = Wall_A(MainGame.SCREEN_WIDTH / 2 - 75, 50 * i)
            MainGame.Wall_A_list.append(wall)
        for i in range(5, 8):
            wall = Wall_A(MainGame.SCREEN_WIDTH / 2 + 25, 50 * i)
            MainGame.Wall_A_list.append(wall)

    # 创建墙壁B的方法
    def creatWalls_B(self):
        for i in range(7, 8):
            wall = Wall_B(MainGame.SCREEN_WIDTH / 2 - 25, 50 * i)
            MainGame.Wall_B_list.append(wall)

        # for i in range(1, 4):
            # wall = Wall_B(MainGame.SCREEN_WIDTH - 450, MainGame.SCREEN_HEIGHT - 50 * i)
            #　MainGame.Wall_B_list.append(wall)
        # for i in range(1, 4):
            # wall = Wall_B(400, MainGame.SCREEN_HEIGHT - 50 * i)
            # MainGame.Wall_B_list.append(wall)
        # for i in range(1, 4):
            # wall = Wall_B(400 + i * 50, MainGame.SCREEN_HEIGHT - 150)
            # MainGame.Wall_B_list.append(wall)

    # 将墙壁A加入到窗口中
    def blitWalls_A(self):
        for wall in MainGame.Wall_A_list:
            if wall.live:
                wall.displayWall_A()
            else:
                MainGame.Wall_A_list.remove(wall)

    # 将墙壁B加入到窗口中
    def blitWalls_B(self):
        for wall in MainGame.Wall_B_list:
            if wall.live:
                wall.displayWall_B()
            else:
                MainGame.Wall_B_list.remove(wall)
    ...
    # 左上角文字绘制的功能
    def getTextSurface_L(self, text):
        # 初始化字体模块
        pygame.font.init()
        # 查看系统支持的所有字体
    ...
    # 右上角文字绘制功能
    def getTextSurface_R(self, text):
        pygame.font.init()
        font = pygame.font.SysFont('微软雅黑', 20, bold=True, italic=False)
        # 绘制内容
    ...
    def getEvent(self):
        ...
        for event in eventList:
            ...
            if event.type == pygame.KEYDOWN:
                #点击 'J' 按键让我方坦克重生，如果还有剩余我方坦克时运行
                if event.key == pygame.K_j and not MainGame.TANK_P1:
                    if MainGame.MyTank_live > 0:
                        #调用创建我方坦克的方法
                        self.creatMyTank()
                        MainGame.MyTank_live -= 1
